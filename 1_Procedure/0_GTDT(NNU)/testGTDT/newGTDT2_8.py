...
expName = 'newGTDT2'  # from the Builder filename that created this script
expInfo = {
...
end = parallel.ParallelPort(address='0x5FB8')
haveARest = visual.ImageStim(
    win=win,
    name='haveARest', 
    image='D:/Desktop/GT/instruction1.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.5, 1),
...
for thisBlock in blocks:
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if background.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > background.tStartRefresh + asec-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    _key_resp_4_allKeys = []
    # keep track of which components have finished
    restComponents = [end, haveARest, key_resp_4]
    ...
    while continueRoutine:
        ...
        if end.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.addData('end.started', t)
            # update status
            end.status = STARTED
            end.status = STARTED
            win.callOnFlip(end.setData, int(bmarker))
        
        # if end is stopping this frame...
        if end.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > end.tStartRefresh + 0.1-frameTolerance:
                # keep track of stop time/frame for later
                end.tStop = t  # not accounting for scr refresh
                end.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('end.stopped', t)
                # update status
                end.status = FINISHED
                win.callOnFlip(end.setData, int())
        
        # *haveARest* updates
        
        # if haveARest is starting this frame...
        if haveARest.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            haveARest.frameNStart = frameN  # exact frame index
            haveARest.tStart = t  # local t and not account for scr refresh
            haveARest.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(haveARest, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'haveARest.started')
            # update status
            haveARest.status = STARTED
            haveARest.setAutoDraw(True)
        
        # if haveARest is active this frame...
        if haveARest.status == STARTED:
            # update params
            pass
