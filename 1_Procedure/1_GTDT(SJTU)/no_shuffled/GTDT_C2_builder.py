...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=root_dir + 'GTDT_C2_builder.py',
...
# set refreshRate = 85Hz
#win.setFlip(clearBuffer=True, refreshRate=85)
...
i3 = visual.ImageStim(
    win=win,
    name='i3', 
    image=root_dir + 'instruction3C2.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.6, 1),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
blocks = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions(root_dir + 'blist_c2.xlsx'),
...
for thisBlock in blocks:
    ...
    for thisComponent in fixationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    if bstart.status == STARTED:    # diff
        win.callOnFlip(bstart.setData, int(0)) #diff
