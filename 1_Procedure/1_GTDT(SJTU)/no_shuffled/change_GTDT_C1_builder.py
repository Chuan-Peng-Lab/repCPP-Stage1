...
expName = 'GTDT1'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=root_dir + 'change_GTDT_C1_builder.py',    
...
i3 = visual.ImageStim(
    win=win,
    name='i3', 
    image=root_dir + 'instruction3C1.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(24, 15),
...
def responses_marker(value):
    ...
    responses.setData(0)
#responses_marker(2)
square = parallel.ParallelPort(address='0x3EFC')
def square_marker(value):
...
i4 = visual.ImageStim(
    win=win,
    name='i4', 
    image=root_dir + 'instruction4.png', mask=None, anchor='center',
...
blocks = data.TrialHandler(nReps=1.0, method='fullRandom', 
    ...
    trialList=data.importConditions(root_dir + 'blist_c1.xlsx'),
...
for thisBlock in blocks:
    ...
    # update component parameters for each repeat
    # keep track of which components have finished
    fixationComponents = [text, bstart] # diff
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if bstart.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.addData('bstart.started', t)
            # update status
            bstart.status = STARTED
            bstart.status = STARTED
            win.callOnFlip(bstart.setData, int(bseq))
        ...
        if bstart.status == STARTED:
            ...
            if tThisFlipGlobal > bstart.tStartRefresh + 0.1-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.addData('bstart.stopped', t)
                # update status
    ...
    trials = data.TrialHandler(nReps=1.0, method='fullRandom', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(root_dir + 'c1.xlsx'),
    ...
    for thisTrial in trials:
        ...
        annular13,annular14,annular15,annular16, cover, center, press, stimulus] # diff
        ...
        # --- Run Routine "GTDT" ---
        routineForceEnded = not continueRoutine
        count = 0 # count the appearance of increased sqaure
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
    ...
    _key_resp_4_allKeys = []
    # keep track of which components have finished
    restComponents = [i4, key_resp_4, bend]
